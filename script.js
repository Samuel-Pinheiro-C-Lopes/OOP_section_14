...
// extends make it a subclass and adds the super() method that will substitute
// the Class.call(this, [...]) way of setting the properties - if there are no extra
// properties, it can be omited but once there are, it should be called first in the constructor
class _StudentCl extends _PersonCl {
    constructor(fullName, birthYear, course) {
        // Always needs to happen first - this call is responsible for
        // creating the this keyword in the subclass
        super(fullName, birthYear);

        this.course = course;
        // if there wasn't any new property, the constructor could be omited,
        // and the super funciton would be called automatically
    }
    ...
    // Polymorphism at work here, because of the prototype chain...
    calcAge() {
        console.log(`I'm ${2037 - this.birthYear} years old, but as a student I feel more
        like ${2037 - this.birthYear + 10} years old...`);
    }
}

const martha = new _StudentCl('Martha Jones', 2012, 'Computer Science');
martha.calcAge();
martha.introduce();
...
// Prototype chain between prototypes - or "class" inheritance - in JS with Object.create();
...
const _steven = Object.create(_PersonProto);
// * _steven.init('Steven', 1674);

// Here this object is created and his prototype is set to be linked with the _PersonProto_ prototype
const _studentProto = Object.create(_PersonProto);
console.log("Should be true 1: ", _studentProto.prototype === _PersonProto.prototype);

_studentProto.init = function (firstName, birthYear, course) {
    _PersonProto.init.call(this, firstName, birthYear);
    this.course = course;
}

_studentProto.introduce = function () {
    console.log(`My name is ${this.firstName} and I study ${this.course}.`);
}


const marthir = Object.create(_studentProto);

marthir.init('Marthir', 2010, 'Computer Science');
marthir.introduce();
marthir.calcAge();

// With the Object.create() the object has it's prototype property linked directly to it's class .prototype and, as such, it's
// class has it's .prototype property linked to it's own parent - since it is a subclass
console.log("Should be true 2: ", marthir.prototype  === _studentProto.prototype
    && _studentProto.prototype === _PersonProto.prototype 
    && marthir.prototype === _PersonProto.prototype // yes, in this case the marthir.prototype is directly linked to _PersonProto.prototype
    && marthir.prototype === Object.prototype // this should be the case as well...
);


// With constructor functions the objects have .prototype undefined and the .__proto__ points to the .prototype of it's class,
// and the .prototype.__proto__ of this class points to the prototype of it's parent class, as such...
console.log("Should be true 3:", mike.__proto__ === _Student.prototype && _Student.prototype.__proto__ === _Person.prototype);
console.log("Should be true 4:", mike.__proto__.__proto__ === _Person.prototype);
console.log("Should be true 5:", _Student.prototype.__proto__.__proto__ === Object.prototype);

// Now that the differences between the constructor functions prototype chain and Object.create are fairly evident, let's see
// what happens with the ES6 classes... - remember: _PersonCl, _StudentCl and Martha are declared and initialized before in this code...

// As it seems, it works the same as the constructor function - since it's exactly what happens behind the scenes...
console.log("Should be true 6: ", martha.__proto__ === _StudentCl.prototype);
console.log("Should be true 7: ", martha.__proto__.__proto__ === _PersonCl.prototype);
console.log("Should be true 8: ", martha.__proto__.__proto__.__proto__ === Object.prototype);
console.log("Should be true 9: ", _StudentCl.prototype.__proto__ === _PersonCl.prototype);
console.log("Should be true 10: ", _StudentCl.prototype.__proto__.__proto__ === Object.prototype);

// The only difference in structure resides in Object.create() structure that doesn't make use of the __proto__ to link prototypes,
// making those link directly

console.dir(mike.prototype);

////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// ANOTHER CLASS EXAMPLE & ENCAPSULATON

// For now, only Google Chrome may fully support those
// 1) Public fields;
// 2) Private fields;
// 3) Public methods;
// 4) Private methods.

// aside from these 4, there are also the static versions
// static public works by simply putting static before the method...

class Account {
    // 1) public fields - added to the instances, not prototypes
    locale = navigator.language;
    // _movements = [];
    
    // 2) private fields
    #movements = [];
    #pin; // that's the case with properties 
    // setted by the cnstructor

    constructor(owner, currency, pin) {
        // constructor can have inner logic in ways as show bellow
        this.owner = owner;
    ...
    // API, public interface of the object
    // better than have to manually manipulate the important properties
    // the withdraw also abstracts the fact that the withdraw is a negative value,
    // abstracting work for the user

    // 3) Those are all Public methods, since they are the 
    ...
    // 4) Private methods - at the time of the 
    // class, this doesn't work in any browser...
    // * #approveLoan(val) {
    _approveLoan(val) {
        return true;
    }
...
// protected properties and methods can still be accessed
// but it's marked as a protected - not to be modified or
// accessed directly, only with the API...
...
// * acc1.approveLoan(1000); // this shouldn't be accessible from outside
console.log(acc1.getMovements());

console.log(acc1);
console.log(acc1.pin); // shouldn't be acessible from outside

// * console.log(acc1.#movements);
// * console.log(acc1.#pin);
// both above doesn't work since those properties are private
...
// doesn't work because the deposit method returns undefined
acc1.deposit(300).deposit(500)
.withdraw(35).requestLoan(25000).withdraw(4000);
...
class CarCl {
    ...
    accelerate() {
        this.speed += 10;
        console.log(`The ${this.make} became faster! 
            It's now running at ${this.speed} km/h`);
        return this;
    }

    brake() {
        this.speed -= 5;
        console.log(`The ${this.make} slowed down! It's now 
            running at ${this.speed} km/h.`);
        return this;
    }
...
class EVcl extends CarCl {
    ...
    accelerate() {
        ...
        this.#charge = Number.parseInt(this.#charge) - 1 + '%';
        console.log(`The ${this.make} became faster! 
            It's now running at ${this.speed} km/h with ${this.#charge}
...
// testing if the challenge is indeed
console.log(rivian.speedUS);
rivian.accelerate().accelerate().accelerate().brake().brake().brake();
console.log(rivian.speedUS);
rivian.speedUS = 80;
rivian.accelerate().brake().brake().brake();
rivian.chargeBattery(30).accelerate();
